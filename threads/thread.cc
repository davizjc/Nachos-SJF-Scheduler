...
Thread::Yield ()  //new one
{
    ...
    if (nextThread != NULL) {
    DEBUG(dbgSJF,"[YS] Tick["<<kernel->stats->totalTicks<<"]:"<<"Thread["<<nextThread->getID()<<"] is now selected for execution, thread[" << kernel->currentThread->getID()<<"],"<<"is replaced, and it has executed ["<<kernel->currentThread->getRunTime()<<"] ticks");
...
void Thread::Sleep(bool finishing)
{
    ...
    DEBUG(dbgSJF,"[S] Tick["<<kernel->stats->totalTicks<<"]:"<<"Thread["<<nextThread->getID()<<"] is now selected for execution, thread[" << kernel->currentThread->getID()<<"],"<<"is replaced, and it has executed ["<<kernel->currentThread->getRunTime()<<"] ticks");
    ...
    if(!finishing) {
        ...
        this->RunTime = 0;
        kernel->scheduler->ReadyToRun(this);
        DEBUG(dbgSJF, "[U]Tick["<<kernel->stats->totalTicks<<"]:"<<"Thread ["<< this->getID()<<"] update approximate burst time, from: ["<< prev_burstTime/2<<"]"<<"+["<< BurstTime/2<<"],to["<<this->getPredictedBurstTime()<<"]");
    }
